...
- Convenience integration:
  - When the point is inside a context block and no region is active, the following commands MUST behave exactly like “Add” above:
    - context-navigator-add-from-text (§13.1)
    - context-navigator-add-universal (§13)
  - Outside a block, or when a region is active, these commands preserve their normal behavior.
